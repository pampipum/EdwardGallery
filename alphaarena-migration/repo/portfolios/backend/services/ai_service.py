...
def get_ai_analysis(ticker: str, technical_data: dict, news: list = [], macro_data: dict = {}, trade_setup: dict = None, current_position: str = "None", strategy = None) -> dict:
    """
    Generates psychological analysis and deep reasoning using Gemini, considering technicals, news, and macro data.
    strategy: PMStrategy instance to customize the AI's behavior
    """
    if not API_KEY:
        return {
    ...
    # Get strategy-specific modifications
    strategy_modifier = ""
    if strategy:
        strategy_modifier = f"\n{strategy.get_prompt_modifier()}\n"
    ...
    - You must decide the `allocation_percentage` based on your CONFIDENCE (0-10).
    - Low Confidence (< 7): 0.0 (Do not trade)
    - Medium Confidence (7-8.5): 0.05 - 0.08 (5-8%)
    - High Confidence (> 8.5): 0.10 - 0.15 (10-15%)
